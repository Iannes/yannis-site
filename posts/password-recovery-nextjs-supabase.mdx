...
export async function GET(req: Request) {
  ...
  if (!token_hash || type !== "email") {
	...
	// Here is were we verify the token_hash so that our users will later be able to update their password.
  const {
    data: { session },
    error,
  ...
  // Redirect to the specified URL which in our case is the app/password-reset/page.tsx
